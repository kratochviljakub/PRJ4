...
\end{eqnarray*}
kde \(T\) \(p_i \in \mathbb{R}^3, i=1 \dots n \) odpovídá pozicím sledovaných snímků \(1 \dots n\), \(S \subset [1;n]\) a \(E \subset [1;n]\) jsou indexy snímků startovní a koncové části trajektorie, pro kterou existují pozice \(\hat{p} \in \mathbb{R}^3\) z ground-truth. \(Sim(3)\) se rozumí 3D podobnostní transformace, ta zahrnuje 7 stupňů volnosti - tři posuny ve směru souřadnicových os, tři rotace podle souřadnicových os jednu změnu měřítka. Nutností je, aby \textit{E} i \textit{S} obsahovaly dostatečné množství pozic a dobře tak vyrovnaly počáteční \uv{zmatený} kalibrační pohyb kamery. Nahromaděnou chybu nyní lze vypočítat jako 
\begin{eqnarray*} 
