...
\begin{titlepage}
\begin{center}
\includegraphics[scale=0.5]{img/logo_zcu}\\
\vspace{5cm}
\begin{Large}
\textbf{\thetitle}\\
\end{Large}
...
Klíčová myšlenka tohoto algoritmu je odhadnout polo-hustou inverzní hloubkovou mapu pro aktuální obraz, která může být použita pro odhad pozice kamery u následujícího snímku. Tato mapa se průběžně šíří ze snímku do snímku a zpřesňuje se novými hloubkovými měřeními, která jsou získávána při adaptivním prostorovém srovnávání pixelů. To umožňuje přesně odhadnout hloubku blízkých i vzdálených částí obrazu.

\paragraph{Aktualizace hloubkové mapy}
Celková aktualizace hloubkové mapy se provádí jednou pro každý nový snímek a skládá se z následujících kroků: Nejprve je vybrána podmnožina obrazových bodů, pro kterou je přesnost vyhledání rozdílu dostatečně velká. Pro každý pixel se pak jednotlivě vybere vhodný referenční rámec a provede se jednorozměrné hledání rozdílnosti. Dřívější poznatky, pokud je to možné, se používají ke snížení rozsahu rozdílového vyhledávání a tím zmenšení výpočetních nákladů a odstranění falešných minim. Získaný inverzní hloubkový odhad se poté spojí do hloubkové mapy.

\textsf{\textbf{TODO-----------------------------------------------------------}}

\subsection{LSD-SLAM}
Metoda LSD-SLAM (Large-Scale Direct monocular SLAM) umožňuje vytvářet konzistentní rozsáhlé mapy prostředí. Používá přímé zpracování vstupního obrazu spolu s odhadem založeným na filtraci hloubkových map. Globální mapa je reprezentována jako graf, jehož vrcholy představují vstupní obrazy a 3D transformace podobnosti znázorňují hrany umožňující detekci a opravu nahromaděné chyby. Metoda běží v reálném čase na CPU a jako odometrie i na smartphonu.

\subsubsection{Přehled metody}
Algoritmus se skládá ze tří hlavních částí:
\begin{itemize}
	\item \textbf{Sledování}; \textsf{\textbf{TODO-----------------------------------------------------------}}
	\item \textbf{Odhad hloubkové mapy}; využívá sledované snímky buď k úpravě nebo nahrazení aktuálního snímku. \textsf{\textbf{TODO-----------------------------------------------------------}}
	\item Optimalizace mapy
\end{itemize}
